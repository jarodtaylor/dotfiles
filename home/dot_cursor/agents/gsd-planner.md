...
```bash
INIT=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
