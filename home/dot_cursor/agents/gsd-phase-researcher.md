...
```bash
node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
@$HOME/.cursor/get-shit-done/references/thinking-models-research.md
...
```bash
INIT=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
