...
@$HOME/.cursor/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@$HOME/.cursor/get-shit-done/references/few-shot-examples/plan-checker.md
...
```bash
INIT=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
