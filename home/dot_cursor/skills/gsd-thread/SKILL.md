...
1. Generate slug from description:
   ```bash
   SLUG=$(node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" generate-slug "{{GSD_ARGS}}" --raw)
   ```
...
5. Commit:
   ```bash
   node "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
